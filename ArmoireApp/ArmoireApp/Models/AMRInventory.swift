...
func getItemsForCategory(category:String, completion: ([AMRInventoryItem]) -> ()) {
  let urlString = "http://shop.nordstrom.com/FashionSearch.axd?category=\(category)&contextualsortcategoryid=0&instoreavailability=false&page=1&pagesize=100&partial=1&sizeFinderId=2&type=category"
  let url = NSURL(string: urlString)
  let request = NSURLRequest(URL:url!)
