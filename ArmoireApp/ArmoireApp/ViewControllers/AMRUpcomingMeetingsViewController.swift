...
class AMRUpcomingMeetingsViewController: AMRViewController {
  ...
  private func scrollToLatestDate(){
    ...
    internal func setUpNavBar(){
      if CurrentUser.sharedInstance.user?.isStylist == true && client != nil {
        title = "Meetings with " + (client?.firstName)!
      } else {
    ...
    func createNewEvent(sender: UIBarButtonItem){
      ...
      newEventVC.event?.title = "Personal Shopping Appointment: " + (stylist?.firstName)! + "-" + (client?.firstName)!
